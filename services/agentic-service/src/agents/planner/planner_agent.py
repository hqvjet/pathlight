...
class PlannerAgent(BaseAgent):
    ...
    async def __call__(self, state: State) -> State:
        print(f"[Planner Agent] Invoked with state: {state}")
        history: List = [
        ...
        ai: AIMessage = await self.chain.ainvoke({"id": state.id, "history": history, "difficulty": state.difficulty, "duration": str(state.duration)})
        history.append(ai)
        print(f"[Planner Agent] - First response: {ai}")
        ...
        while ai.tool_calls:
            ...
            for tool_call in ai.tool_calls:
                ...
                ai = await self.chain.ainvoke({"id": state.id, "history": history, "difficulty": state.difficulty, "duration": str(state.duration)})
                count += 1
                print(f"[Planner Agent] - Response {count}: {ai}")

        print(f"[Planner Agent] - Final response: {ai.content}")
        json_content = json.loads(ai.content)
        print(f"[Planner Agent] - Final response: {json_content}")
