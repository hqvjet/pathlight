...
def get_course_stats(user_id: str, user_email: Optional[str] = None) -> dict:
    ...
    try:
        ...
        url = f"{base}/all"
        logger.info(f"[COURSE_CLIENT] Fetching course stats from {url} for user_id={user_id} email={user_email}")
        logger.info(f"[COURSE_CLIENT] COURSE_SERVICE_URL = {config.COURSE_SERVICE_URL}")
        
        courses_resp = requests.get(url, headers=_headers(user_id, user_email), timeout=DEFAULT_TIMEOUT)
        ...
        total_lessons = sum(c.get("lesson_num", c.get("num_lessons", 0)) for c in courses_list)
