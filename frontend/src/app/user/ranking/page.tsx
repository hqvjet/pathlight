    ...
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 via-blue-50 to-indigo-50 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-gray-600">Xem thứ hạng của bạn trong cộng đồng học tập</p>
        </div>
        ...
        {!loading && user && (
          <div className="mb-8 bg-gradient-to-br from-cyan-600 to-blue-700 rounded-2xl shadow-2xl p-6 text-white">
            <div className="grid md:grid-cols-3 gap-6">
              {/* User Info */}
              <div className="md:col-span-1 flex flex-col items-center justify-center text-center border-r border-white/20">
                <div className="text-6xl font-black mb-2">
                  #{user.rank || '—'}
                </div>
                <div className="text-sm text-cyan-100">Thứ hạng của bạn</div>
              ...
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="grid grid-cols-3 gap-3 pt-2">
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">{user.completed_courses || 0}</div>
                    <div className="text-xs text-cyan-100">Khóa học</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">{user.current_exp || 0}</div>
                    <div className="text-xs text-cyan-100">Kinh nghiệm</div>
                  </div>
                  <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 text-center">
                    <div className="text-xl font-bold">{user.total_lessons || 0}</div>
        ...
        {!loading && competitors && (
          <div className="mb-6 bg-gradient-to-br from-orange-50 to-amber-50 rounded-2xl shadow-xl p-6 border-2 border-orange-200">
            <div className="flex items-center gap-3 mb-4">
              <Users className="w-6 h-6 text-orange-600" />
              <h3 className="text-xl font-bold text-gray-900">Đối thủ xung quanh bạn</h3>
            </div>
            <div className="space-y-3">
                ...
                <div className="bg-white rounded-lg p-4 border-2 border-orange-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-2xl font-black text-orange-600">#{competitors.above.rank}</div>
                      <div>
                        <div className="font-semibold text-gray-900">{competitors.above.name}</div>
                        <div className="text-sm text-gray-600">Lv {competitors.above.level} • {(competitors.above.experience || 0).toLocaleString()} EXP</div>
                      </div>
                    </div>
              ...
              <div className="bg-gradient-to-r from-cyan-500 to-blue-600 rounded-lg p-4 border-2 border-cyan-400 shadow-lg">
                <div className="flex items-center justify-between text-white">
                  <div className="flex items-center gap-3">
                    <div className="text-2xl font-black">#{competitors.current.rank}</div>
                    <div>
                      <div className="font-bold text-lg">{competitors.current.name} (Bạn)</div>
                      <div className="text-sm text-cyan-100">Lv {competitors.current.level} • {(competitors.current.experience || 0).toLocaleString()} EXP</div>
                    </div>
                  </div>
                </div>
              </div>
              {competitors.below && (
                <div className="bg-white rounded-lg p-4 border-2 border-gray-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="text-2xl font-black text-gray-600">#{competitors.below.rank}</div>
                      <div>
                        <div className="font-semibold text-gray-900">{competitors.below.name}</div>
                        <div className="text-sm text-gray-600">Lv {competitors.below.level} • {(competitors.below.experience || 0).toLocaleString()} EXP</div>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        {!loading && users.length > 0 && (
          <div className="space-y-6">
            {/* Top 3 Podium */}
            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border-2 border-amber-200">
              <div className="flex items-center justify-center gap-2 mb-6">
                <span className="text-3xl">👑</span>
                <h2 className="text-2xl font-bold text-gray-900">Top 3 Xuất Sắc</h2>
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-50 to-cyan-50 rounded-xl p-6 border border-blue-200">
              <div className="flex items-start gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 mb-2">Cách tăng thứ hạng</h3>
